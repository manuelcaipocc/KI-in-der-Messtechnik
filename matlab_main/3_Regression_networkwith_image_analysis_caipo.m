...
% loader = SpiralDataLoader(csvPath, imageFolder, [128 128]);
% 
% X = loader.getImageArray();  % [128 x 128 x 3 x N]
% X = im2double(X);
% Ypos = loader.Positions';    % [3 x N]
% Ycolor = double(loader.Colors)'; % [1 x N]
% Y = [Ypos; Ycolor];          % [4 x N]
% 
% % --- Separar entrenamiento y prueba ---
% N = size(X, 4);
% idx = randperm(N);
% train_count = round(0.8 * N);
% Xtrain = X(:, :, :, idx(1:train_count));
% Ytrain = Y(:, idx(1:train_count));
% Xtest = X(:, :, :, idx(train_count+1:end));
% Ytest = Y(:, idx(train_count+1:end));
% 
% % --- Inicialización CNN ---
% % Conv layer: 8 filtros 3x3x3
% filt = randn(3, 3, 3, 8) * 0.01;
% bias = zeros(8, 1);
% 
% % FC1: 63x63x8 = 31752
% W1 = randn(64, 31752) * 0.01;
% b1 = zeros(64, 1);
% W2 = randn(4, 64) * 0.01;
% b2 = zeros(4, 1);
% 
...
% for epoch = 1:epochs
%     % --- Forward ---
%     conv1 = conv2d_layer(Xtrain, filt, bias);        % [126x126x8xN]
%     relu1 = relu3d(conv1);                           % ReLU
%     pool1 = maxpool2d(relu1, 2);                     % [63x63x8xN]
%     flat = reshape(pool1, [], size(pool1, 4));       % [31752 x N]
% 
%     A1 = relu_func(W1 * flat + b1);                  % FC1
%     A2 = W2 * A1 + b2;                               % FC2
%     A2(end, :) = sigmoid_func(A2(end, :));           % sigmoid para color
% 
%     loss = mean((A2 - Ytrain).^2, 'all');
% 
%     % Backward solo en FC layers
%     dZ2 = 2 * (A2 - Ytrain);
%     dZ2(end,:) = dZ2(end,:) .* dsigmoid_func(A2(end,:));
%     dW2 = dZ2 * A1' / train_count;
%     db2 = sum(dZ2, 2) / train_count;
% 
%     dA1 = W2' * dZ2;
%     dZ1 = dA1 .* drelu_func(W1 * flat + b1);
%     dW1 = dZ1 * flat' / train_count;
%     db1 = sum(dZ1, 2) / train_count;
% 
%     % --- Gradient descent ---
...
% 
% % --- Inference ---
% conv1 = conv2d_layer(Xtest, filt, bias);
% relu1 = relu3d(conv1);
% pool1 = maxpool2d(relu1, 2);
% flat = reshape(pool1, [], size(pool1, 4));
% 
% A1 = relu_func(W1 * flat + b1);
% A2 = W2 * A1 + b2;
% A2(end,:) = sigmoid_func(A2(end,:));
% 
% Ypred = A2;
% Ypred_pos = Ypred(1:3, :);
% Ypred_color = Ypred(4, :) > 0.5;
% 
% Ytrue_pos = Ytest(1:3, :);
% Ytrue_color = Ytest(4, :);
% 
% pos_error = mean(vecnorm(Ypred_pos - Ytrue_pos));
% acc_color = mean(Ypred_color == Ytrue_color);
% 
% fprintf('Pos Error: %.2f\n', pos_error);
% fprintf('Color Acc: %.2f %%\n', acc_color * 100);
% 
% % --- Funciones auxiliares ---
% function y = relu_func(x), y = max(0, x); end
% function dy = drelu_func(x), dy = double(x > 0); end
% function y = sigmoid_func(x), y = 1 ./ (1 + exp(-x)); end
% function dy = dsigmoid_func(x)
%     s = sigmoid_func(x); dy = s .* (1 - s);
% end
% 
% function out = conv2d_layer(X, F, B)
%     [H,W,C,N] = size(X); [fH,fW,~,Fnum] = size(F);
%     out = zeros(H - fH + 1, W - fW + 1, Fnum, N);
%     for n = 1:N
%         for f = 1:Fnum
%             tmp = 0;
%             for c = 1:C
%                 tmp = tmp + conv2(X(:,:,c,n), F(:,:,c,f), 'valid');
%             end
%             out(:,:,f,n) = tmp + B(f);
%         end
%     end
% end
% 
% function out = relu3d(X), out = max(0, X); end
% 
% function P = maxpool2d(X, pool)
%     [h, w, c, N] = size(X); oh = floor(h/pool); ow = floor(w/pool);
%     P = zeros(oh, ow, c, N);
%     for n = 1:N
%         for ch = 1:c
%             for i = 1:oh
%                 for j = 1:ow
%                     reg = X((i-1)*pool+1:i*pool,(j-1)*pool+1:j*pool,ch,n);
%                     P(i,j,ch,n) = max(reg(:));
%                 end
%             end
%         end
