...
% Fehleranzeige mit Input-Werten
disp('--- TESTERGEBNISSE ---');
fprintf('%6s  %20s  %12s  %12s  %12s\n', 'Test-Nr', 'Input-Werte', 'Erwartet', 'Vorhersage', 'Absoluter Fehler');
for i = 1:10
    fehler = abs(Y_true(i) - Y_pred(i));
    input_str = sprintf('[%d %d %d %d %d]', X_test(i,1), X_test(i,2), X_test(i,3), X_test(i,4), X_test(i,5));
    fprintf('%6d  %20s  %12.2f  %12.2f  %12.2f\n', i, input_str, Y_true(i), Y_pred(i), fehler);
end
